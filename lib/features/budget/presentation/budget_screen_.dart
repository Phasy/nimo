...
Future<void> _showAllocationDialog({
  ...
  final controller = TextEditingController(
    text: category.assigned == 0
        ? ''
        : Money.formatMinorUnits(category.assigned),
  );
      ...
      return StatefulBuilder(
        builder: (context, setState) {
          Future<void> save() async {
            ...
            if (amount == null) {
              setState(() {
                errorText = 'Enter a valid amount, for example 500 or 1250.50.';
              });
          ...
          return AlertDialog(
            ...
            content: Column(
              ...
              children: [
                Text(
                  'Set the amount assigned to this category for ${_formatMonthShort(summary.year, summary.month)}.',
                ),
                ...
                Text(
                  'Current available: ${_money(category.available)}',
                  style: Theme.of(context).textTheme.bodySmall?.copyWith(
                    color: AppTheme.textSecondary,
                  ),
                ),

                if (category.startingAvailable != 0) ...[
                  const SizedBox(height: 4),
                  Text(
                    'Carried forward: ${_money(category.startingAvailable)}',
                    style: Theme.of(context).textTheme.bodySmall?.copyWith(
                      color: AppTheme.textSecondary,
                    ),
                  ),
